...
class myThread3(threading.Thread):
    ...
    def run(self):
        ...
        image=self.image
        pose_result = e.inference(image,resize_to_default=(w > 0 and h > 0), upsample_size=args.resize_out_ratio)
        if not args.showBG:
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if frame_no ==1:
            ...
            t2.start()  
            t3= myThread3(image,frame_no, workQueue)  # initialise thread to do first frame
            t3.start()  
        ...
        if workQueue.qsize()==3:   # if the queue size is three
            ...
            #print(hand_result,pose_result,face_result)
            t1= myThread1(image,frame_no, workQueue)  # initialise thread to do first frame
            t1.start()  
            t2= myThread2(image,frame_no, workQueue)  # initialise thread to do first frame
            t2.start()  
